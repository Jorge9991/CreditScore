...
class daoTipoUsuario implements Itipousuario {
    ...
    public function crear(\TipoUsuario $tipousuario) {
        try {
            $sql = 'insert into inv_tipousuario (descripcion,estado) values (:descripcion,:estado)';
    ...
    public function delete(\TipoUsuario $tipousuario) {
        try {
            $sql = 'UPDATE inv_tipousuario SET estado=:estado WHERE idTipoUsuario = :id';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
            $stmp->bindParam(':estado', $tipousuario->getEstado(), PDO::PARAM_INT);
    ...
    public function editar(\TipoUsuario $tipousuario) {
        try {
            $sql = 'UPDATE inv_tipousuario SET descripcion=:descripcion, estado=:estado WHERE idTipoUsuario = :id';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
            $stmp->bindParam(':id', $tipousuario->getIdTipoUsuario(), PDO::PARAM_INT);
            $stmp->bindParam(':descripcion', $tipousuario->getDescripcion(), PDO::PARAM_STR);
    ...
    public function listar() {
        $tiposusuarios = array();
        try {
            $sql = 'SELECT * from inv_tipousuario where estado = 1 group by idtipousuario  order by idtipousuario asc ';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
            $stmp->execute();
    ...
    public function listarId($id) {
        $tipousuario = null;
        try {
            $sql = 'SELECT * from inv_tipousuario WHERE idTipoUsuario = :id';
            $cn = DataBase::getInstancia();
            $stmp = $cn->prepare($sql);
